...
fn parse_recursive(node: Node, code: &SourceCode) -> Result<ExpressionSpan, AnyError> {
    let (expression, span) = match node {
        ...
        _ => return err_span(format!("unimplemented: {:?}", node), code, node.span()), // Node::String { .. } => {}
                                                                                       // Node::Identifier { .. } => {}
...
fn construct_keyword(keyword: Keyword, span: Span) -> Result<(Expression, Span), AnyError> {
    let (expr, content_span) = match keyword {
        ...
        Keyword::Macro => unimplemented!(),
        _ => unimplemented!(), // Keyword::Loop => construct_loop(parser),
