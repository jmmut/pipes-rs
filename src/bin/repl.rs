...
use pipes_rs::evaluate::{NOTHING, Runtime};
...
fn interpret<R: Read, W: Write>(args: &Args, read_src: R, print_dst: W) -> Result<(), AnyError> {
    let Args { input_file, evaluate_string, check, no_clear } = args;
    ...
    let code_string = SourceCode::new_from_string_or_file(evaluate_string.clone(), input_file.clone())?;
