...
impl Parser {
    ...
    pub fn parse_tokens(tokens: Tokens) -> Result<Expression, AnyError> {
        ...
        let error_expected = |message, token, currently_parsing| {
            error_expected_actual(&format!("{} (currently parsing: {:?})", message, currently_parsing), token)
        };
        for token in tokens {
            match currently_parsing {
                Parsing::AnyExpression => {
                    match token {
                        ...
                        _ => return error_expected("expression (value, identifier, chain or list)", token, currently_parsing),
                    };
                }
                Parsing::Identifier => {
                    match token {
                ...
                Parsing::TransformationsOperator => {
                    match token {
                        Token::Operator(operator) => {
                            currently_parsing = Parsing::TransformationsOperand(operator);
                        }
                        Token::CloseBrace => {
                            let chain = Expression::Chain { initial: Box::new(working_expressions.pop().unwrap()), transformations };
                ...
                Parsing::TransformationsOperand(operator) => {
                    match token {
                        Token::Number(n) => {
                            transformations.push(Transformation { operator, operand: Expression::Value(n) });
                        currently_parsing = Parsing::TransformationsOperator;
                        }
                        Token::Identifier(name) => {
                            transformations.push(Transformation { operator, operand: Expression::Identifier(name) });
                        currently_parsing = Parsing::TransformationsOperator;
                ...
                Parsing::List => {
                    match token {
                        ...
                        Token::CloseBracket => {
                            let mut previous_working_expressions = working_expressions_stack.pop().unwrap();
                            previous_working_expressions.push(Expression::StaticList { elements: working_expressions });
        ...
        working_expressions.pop().ok_or("unable to parse expression".into())
    }
